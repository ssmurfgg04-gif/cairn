...
| Item | Status | Detail |
|---|---|---|
| fuse-mount-live dispatch (no-hardware path) | ✅ live-mount GREEN | workflow gained a `runner` dispatch input: `ubuntu` = GitHub-hosted VM (**has /dev/fuse**, zero registration) / `self-hosted` = runbook box (CAIRN_FUSE_LIVE-gated). Runbook §0 documents the no-hardware path. First real executions of the non-ignored mount test: run 1 caught `create()` passing the spool fh as the attr **ino** (collides with root ino 1) and leaving the kernel fh 0 — fixed (table-allocated ino + real fh); run 2 reached the Phase-2 EBUSY step and caught `lease_scope` resolving `.cairn-domains` from the store-root DISK while the mount authors it into SYNCED STATE — fixed (store-first + disk fallback); run 3: **live_mount_roundtrip_through_kernel PASS on the real /dev/fuse VM** (daemon-smoke step hit the 20min cold-cache cap → 30min + cache-on-failure; run 4 for the fully-green job) |
...
| Commit chain today | ✅ | 16486b4 (fuse dispatch target) → cc0af59 (create fix + ADR-0015 + watermark test) → ab6fe49 (R2 soak fix + lease_scope synced-state fix + soak preflight) → 01370e3 (30min cap + cache-on-failure); CI watch on 01370e3 |
