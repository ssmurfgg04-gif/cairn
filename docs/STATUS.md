...
| Item | Status | Detail |
|---|---|---|
| release.yml RUSTFLAGS trap | ✅ fixed | v1.0.0 re-tag run 33948944674 FAILED at build: env `RUSTFLAGS: -C target-feature=+crt-static` REPLACES `.cargo/config.toml` rustflags (cargo's documented precedence) so tokio's `io-uring` feature (ADR-0025) lost `--cfg tokio_unstable` and hard-errored — installer-gate never ran. Both build steps now carry `--cfg tokio_unstable -C target-feature=+crt-static` with a comment naming the dead run. cairn-app unaffected (workspace-excluded, own tree) |
| Tray supervises the daemon (ADR-0029) | ✅ | `cairn-tray/src/supervise.rs`: pure state machine (probe → spawn → backoff 8s→15s→30s→60s→120s→300s cap, success resets), unit-tested on every platform (6 tests), wired into the windows poll worker: spawns `cairn daemon` HIDDEN (CREATE_NO_WINDOW), stderr appended to `<home>/daemon.log`, child handle dropped on purpose (the daemon outlives the tray). Daemon-lost balloon now says "restarting it…" instead of "open a terminal" |
| install.ps1 starts the daemon (step 6b) | ✅ | `Start-Process cairn daemon -WindowStyle Hidden` + daemon.log, skipped under CAIRN_INSTALL_NO_LAUNCH — the dashboard is live at 127.0.0.1:17778 the moment install finishes: no terminal, no reboot, no tray click |
| Release gate: daemon + dashboard actually RUN on the installed binary | ✅ new step | installer-gate now starts the installed `cairn.exe daemon` exactly the way the tray does, polls http://127.0.0.1:17778/ for 200 within 60s (daemon log tail on failure), runs `cairn status --json` against the live daemon, cleans up. "Install → daemon runs → dashboard serves" is a tested contract on a clean Windows runner |
| Attach/resume survives server-down (real bug the E2E caught) | ✅ | `resume_all` → `attach` hard-failed on `ensure_project` when the server was unreachable at daemon boot → daemon resumed ZERO projects, sync silently dead (exactly the tray-restart scenario this round builds). Fix: ensure is best-effort in `attach` (binding kept, warn) and moved into `run_loop` after every successful (re)connect — the 5s retry ladder owns recovery. E2E: daemon restart with NO server → 2 live error-state runtimes → server returns → synced,synced |
| Review link --role studio, internal notes E2E (real processes) | ✅ | Real server + daemon + review portal on 127.0.0.1:17779, real 3MB media, publish → studio link → compose: internal comment, public pin (0.42/0.30), internal RANGE note (f120–180), client public note; client link asking internal → 403; studio session JSON: 4 comments (2 internal), client session: exactly the 2 public; browser-verified (studio portal shows INTERNAL badges ×5 markers, client portal 0 internal mentions); `export-markers --visibility public` = 2, `all` = 4 |
| Dashboard vs REAL projects (not the mock) | ✅ + 2 real bugs found & fixed | Real daemon, 2 real file trees (6.4MB media), real sync, real journal → chart peak 8.9MB, recent assets/pins/feed all real. Bugs the mock hid: (1) `th.status`/`th.pinned`/`card.locks`/`card.team` missing from the i18n dictionary → headers rendered "TH.STATUS" raw; (2) the state chip called project-error "daemon unreachable" while the daemon served the page — split into error (red, needs attention) / retry (amber, auto-reconnecting) buckets, DAEMON_UP=false is the only "daemon unreachable". VLM on the real-data dashboard: 9/9/9; portal 9/10, INTERNAL badge unambiguous |
| Gates | ✅ | cargo test --workspace 524 green (+6 supervision); clippy --workspace --all-targets -D warnings clean; fmt clean; node --check app.js; windows-msvc cross-checks (cairn-cli + cairn-tray) compile clean |

## Round 25 (2026-09-05) — the mockup dashboard (hero + real activity chart) + note-shape v2 implemented

| Item | Status | Evidence |
|---|---|---|
| The mockup dashboard | ✅ | `assets/dashboard/{index.html,app.css,app.js}` rebuilt to the user's mockup: warm-beige light theme (`#f7f6f2` canvas, white cards, 8px controls), the topbar sync chip becomes the mockup's green-check "all synced", language moves from the footer into a globe + name dropdown, and the dashboard view gains the hero (centered "Creative Workflow Overview" + dark "Add to Workspace" button + the wide "Workspace path or ID (optional)" input — a path attaches, a bare project id jumps to its Files view, empty opens the add panel). Two panels per the mockup: **RECENT ASSETS** (file / status / pin columns, type-aware icons: film for BRAW/mov, note for wav, doc for EDL; working pin toggles) and **RECENT SESSIONS** (recent session + active-project chips + latest-actions timeline + the activity chart). Vertical discipline held: one hairline-separated section stack, no stub rows, sections fold away when empty. VLM: light 9/9, dark 8/10 (contrast bumped after), zero console errors; functional matrix green (hero input paths, chip→files jumps, pin toggle, language dropdown ×4, theme, chart hover). |
| The REAL activity chart | ✅ | `GET /api/v1/activity?days=&tz_offset=` — per-day byte totals from the files table, day boundaries in the CALLER's timezone (JS `getTimezoneOffset` convention; `Store::daily_activity` in db.rs, BTreeMap-bucketed, `div_euclid` day math, store test incl. a UTC+3 boundary case). The chart is drawn by app.js as real SVG: "nice" decimal y ticks (0/50/100 kB like the mockup's 0k/40k/120k), weekday labels via `Intl.DateTimeFormat` (localized, never a hardcoded array), a Fritsch–Carlson **monotone cubic** line (smooth, cannot overshoot below a data point), gradient area fill, per-day hover columns with a dot + tooltip, today's label emphasized. Empty project → the honest empty note, never a fake curve. The feed endpoint also gained real event kinds: pins (pins table, `recent_pins`), held leases as "opened" (ADR-0014 locks), file upserts as synced/saved — the mockup's three example actions are all real store data. |
| Note-shape v2 (ADR-0028) | ✅ implemented | The protocol round, landed: `NoteAnchor.range` (inclusive, degenerate `[f,f]` = the v1 shape), `NoteKind {Comment,Pin,Annotation}`, `NoteVisibility {Public,Internal}`, `pin: Option<(f32,f32)>`, `attachment: Option<BLAKE3 hex>` — all serde-defaulted so v1 files serialize byte-identically. Id material is versioned: v2 = `blake3("note2"‖anchor‖body‖author‖kind‖range_key‖visibility)`; pin/attachment are identity-NEUTRAL and merge field-wise (one side moved → that side wins; both moved → a surfaced conflict, never silent). CSV gains the v2 columns + `export_visible`; the NLE bridge renders ranges as marker durations and pins/ranges/visibility as marker metadata. **All six ADR acceptance gates are tests**: id partition (proptest ×200), v1-fixture parse + byte-stable serialization, round-trip id stability through merge, range/point bucket semantics, pin field-wise merge, CSV visibility golden, portal boundary (below), attachment I2 (below). |
| Portal visibility boundary + compose | ✅ | `cairn-review`: `GuestRole::Studio` (comments + sees internal), the session handler filters Internal notes BEFORE serialization for client-audience links (a devtools snoop finds nothing — the http test asserts the bytes never ship), the comment endpoint takes the v2 envelope (kind/frame_end/pin/attachment/visibility) and REFUSES client attempts to mint internal (403). The player's composer gains pin / range / internal tools (pin armed → the next frame click captures the normalized position; live pin preview), range notes render as scrub-bar BRACKETS, pin notes as diamond ticks + frame dots, annotation overlays compose over the frame from `GET /r/:token/attachment/:hash` — hash-verified on read (I2: tampered → 422 + the player's "drawing unavailable" affordance, missing → 404, never a crash). The browser smoke test posts a full v2 envelope (kind=pin, pin_x/y, frame_end, visibility=internal) end-to-end. |
| export-markers --visibility | ✅ | CLI + the NLE-panel HTTP endpoint take `--visibility public|internal|all` (default **public** — "what the client gets"); `filter_set` is the one filter feeding OTIO/FCP7/CSV shapes; handoff.rs golden test: internal excluded from public, included in all, range notes carry `<duration>`. |
| Verification | ✅ | `cargo test --workspace` GREEN (**518 tests**, +58: notes v2 suite, id-partition proptest, marker bridge v2, store daily-activity/pins, portal boundary/compose/attachment, export goldens); `clippy --workspace --all-targets -D warnings` clean; `fmt --check` clean; `node --check` on app.js + review.js; mock-daemon dataset matches the mockup (clip1.braw / sc2_tk5_grade.edl / music_track.wav / final_cut.edl, Cairn Video New + Cairn_Ad_Spot + Short_Film_v2, the 7-day Sun–Sat curve with the Friday peak). |

## Round 24 (2026-09-05) — the three-destination dashboard, the BURST read lane, note-shape v2 drafted

| Item | Status | Evidence |
|---|---|---|
| The three-destination redesign | ✅ | `assets/dashboard/{index.html,app.css,app.js}` rewritten: 13 nav items → **Dashboard / Files / Settings**; the taste-skill + minimalist-skill protocols applied (1px hairlines, warm monochrome + muted pastel badges, dark mode as a first-class theme with pre-paint bootstrap, tabular numerals, monospace only for paths/ids/code, zero em-dashes in copy, cascade entry + prefers-reduced-motion). Progressive disclosure: telemetry/flags/locks/team/doctor live in Settings; pins/presence surface as icons where work happens; slide-over panels (add/versions/recall) + toasts replace in-table action rows and window.alert. One state derivation feeds topbar chip + footer dot (the old UI could say "all files synced" beside an ERROR row); Windows `\\?\` roots display without the prefix, full path on hover/copy; journal badges speak human (saved/renamed/deleted); the sparkline is honest (journal bytes bucketed — no invented days-of-week). **VLM review progression vs the old UI: clarity 3→9, trust 4→9, visual calm 1→10; final verdict SHIP; zero console errors; flow tests green (onboarding reveal, session→files, panels, esc, theme, i18n ×4 languages) against the mock daemon.** |
| BURST read lane (ADR-0027) | ✅ | `Cas::get_async`: buffers ≥256 KiB verify BLAKE3 on the rayon CPU lane (spawn + oneshot, buffer ownership round-trips — zero copies, I/O worker never hashes; the PostHog rule runs both directions: small work stays inline). `touch()` debounced through a lock-free 2048-slot `TouchFilter` (60 s window, fixed memory, zero allocation) — kills the serialized `UPDATE blobs SET atime` write storm that every BURST re-open paid. Honest rejections in the ADR: SO_REUSEPORT (single-process accept is not the bottleneck), sim queue bounds (the sim has no async mailboxes — the bound landed on the watcher in Round 22), tokio-uring direct dep (tokio::fs already rides the armed driver). |
| Note-shape v2 (ADR-0028) | 📝 proposed | The named next protocol round, drafted: versioned id material (`note2` tag → no v1/v2 collision, v1 notes never rewrite — lazy migration, zero scripts), optional inclusive frame **ranges** (merge key stays anchor-first), **pins** (kind + normalized on-frame position, maps to the Premiere marker bridge), **annotations** (content-addressed PNG overlay blob in the existing CAS — I2 applies), **per-note visibility** (Public/Internal filtered server-side at the review-portal boundary; exports gain `--visibility`). Acceptance gates enumerated (id partition proptest, v1 fixture parse, merge semantics, portal filter, export goldens, annotation I2). |
| install.ps1 HKCU | ✅ (verified intact) | The Round 21 fix (`New-Item -Force` before the Run-key write) confirmed present and unregressed; the release gate run on the re-tagged v1.0.0 exercised it end-to-end. |
| Verification | ✅ | Full `cargo test --workspace` GREEN (~460 tests incl. 2 new CAS pins: debounce window + rayon-lane round-trip/corruption); `clippy --workspace --all-targets -D warnings` clean; `fmt --check` clean; `node --check` on app.js; dashboard VLM + functional matrix per above. Disk-full mid-round (incremental cache) cleared with CARGO_INCREMENTAL=0 + cache pruning — suite green after. |

## Round 23 (2026-09-05) — mimalloc + the Frame.io review surface + the honest remainder (ADR-0026)

| Item | Status | Evidence |
|---|---|---|
| mimalloc global allocator | ✅ | `cairn-cli` (the shipped binary: CLI+daemon+server) binds `mimalloc::MiMalloc` — per-thread heaps, 8B header, 42ns/128B, p99 89ns, 12-week fragmentation 20.7% vs ~67% class (johal.in Rust-1.85 memory round). Our shape is the cited winner: many small worker-thread-local allocations. |
| Frame.io review round | ✅ (frontend-carried) | `review.js/html/css`: whole-row click → playhead jump (`.seekable`), search (body/#tag/author), #tag chip bar from notes' own hashtags, tc↔newest sort, per-note **Copy Link to timestamp** (`?t=&v=` deep links that seek on load), **filtered CSV export**. The note id (blake3 of anchor‖body‖author, ADR-0018) stays FROZEN — range I/O, pins, annotation canvas, per-note internal/public need a persisted note-shape v2: recorded as the named follow-up (ADR-0026), not body-text hacks. |
| Store pragmas | ✅ | `cache_size=-32000` (32 MiB page cache for multi-MB blob serves) + `temp_store=MEMORY` on the client store open (sqlite-kit convention; WAL/busy_timeout/NORMAL already held). |
| Honest remainder | ✅ ADR-0026 | chunkrs/Bytes/NEON swap REJECTED (chunk identities are protocol-frozen — swapping chunkers breaks every id); QUIC/quinn+zblob deferred (wire-protocol re-plumb; p2p swarm is the right future home); zstd 3→7 deferred (wire-only, -3 is the deliberate tradeoff); PGO/BOLT/UPX/distroless/seccomp recorded as tail; FUSE mount flags queued for the live runner. |
| Verification | ✅ | `cargo check`/`clippy -D warnings`/`fmt --check` clean on the touched crates; store+review suites green; JS syntax-checked (`node --check`). |

## Round 22 (2026-09-05) — runtime discipline: the PostHog lane, io_uring, r2d2 readers, the devx ten (ADR-0025)

| Item | Status | Evidence |
|---|---|---|
| PostHog CPU-offload lane | ✅ | `cairn-sync/src/offload.rs`: hash+chunk (FastCDC+BLAKE3, ~1 GiB/s) no longer runs on tokio I/O workers — small work (<8 MiB) inline, big work `rayon::spawn`+oneshot, semaphore-sized-to-lanes pressure valve, boot budget (rayon = all cores, tokio workers = half, min 2). Pinned by `offload_keeps_the_io_worker_free`: on a 1-worker runtime a 96 MiB ingest must not delay an unrelated 20 ms timer past 3.5× slack (pre-lane code fails by ~100 ms). Bit-identical results pinned (`big_buffers_offload_identically`: spans/hashes/stream/file-hash equal the inline pass); raw-size idempotency key + raw-file header carve preserved in `process_file` (captured before the buffer moves). |
| io_uring, honestly unstable | ✅ | Workspace tokio → `features=["full","io-uring"]` (lockfile 1.53.1 — the feature ships there; requires ≥1.53), `--cfg tokio_unstable` set for EVERY build via `.cargo/config.toml` (tokio compile_errors without it — a loud guard). No hand-written uring: `tokio::fs::read` (engine reads + `Cas::get_async`) rides tokio's driver, **runtime-probed with automatic fallback** (this sandbox's 5.10 kernel is below the 5.15 comfort line; suite green either way). Windows/macOS compile clean — verified cross-check `x86_64-pc-windows-msvc` (tokio's io-uring dep is `cfg(all(tokio_unstable, linux))`-gated); `enable_all()` arms it with zero uring-specific code. SQPOLL: not exposed by tokio 1.53 — documented, not pretended. |
| r2d2 reader pool (WO6-5 done properly) | ✅ | `HeaderCache::with_read_pool` = r2d2 `Pool<SqliteConnectionManager>` (query_only, busy_timeout 5s), **8 wide** everywhere production serves (FUSE `for_project_device`, daemon `run_loop`, burst bench agree), `try_get` non-blocking with shared-connection fallback (pool = optimization, never a dependency — pinned by `saturated_pool_falls_back_to_the_shared_connection` + the >1-peak concurrency test). Kills the old hand-rolled pool's unbounded top-up pathology. `r2d2_sqlite` pinned 0.25.x = the rusqlite-0.32 line (probed: 0.31→rusqlite 0.37 would duplicate the crate). |
| Bounded watcher mailbox | ✅ | The daemon's one unbounded channel (`projects.rs` watcher feed) → `mpsc::channel(512)` with forwarder-thread backpressure (`blocking_send`) + a ≥80 %-saturation warn (and drain notice) in the consumer — the "strictly bounded mailboxes" discipline with the budget spent loudly. |
| The devx ten | ✅ | (1) toolchain components auto-installed (`rust-toolchain.toml`: clippy/rustfmt/rust-analyzer/rust-src; `rust-version` 1.85) · (2) `just check-fast` + rust-analyzer on-save · (3) clang+lld/mold block in `.cargo/config.toml`, opt-in with the RUSTFLAGS-precedence warning · (4) sccache documented (local install + exact CI snippet; CI keeps rust-cache — ADR-0025 records why) · (5) profiles: dev incremental-on (CI pins CARGO_INCREMENTAL=0), release opt3/fat-LTO/cgu1/panic-abort/strip · (6) `just machete` · (7) `just timings` + CI `timings` job uploading the HTML report per push · (8) workspace already 17 crates (documented) · (9) `.vscode/` clippy-on-save + recommended extensions · (10) nextest (CI runner; local `just test`). `docs/DEVELOPING.md` is the map. |
| Verification | ✅ | Workspace `cargo check --all-targets` clean, `clippy --workspace --all-targets -D warnings` clean, `fmt --check` clean, full `cargo test --workspace` GREEN (~480 tests incl. the 5 new offload pins + 2 new pool pins), windows-msvc cross-check green. Burst re-run for the 8-wide pool: BENCHMARKS.md (lockstep series expected to drop; the gated CfAPI-parity series keeps its headroom). |
...
| Audit ledger (local, honest) | ✅ | `.cairn/audit.json` — durable + bounded + content-derived ids; dashboard Team tab renders the trail. HONEST SCOPE: `.cairn*` is ignore-listed (SPEC §10), so the ledger is per-machine — the round-16 "review state syncs" claim never held at the transport; synced state is the named follow-up (ADR-0022 consequences). |
...
| W4 convergence catch (post-round CI) | ✅ | nle-matrix run 33891087316: W4 RED — A refused B's post-save upsert as a "stat drift" and held v1 forever (silent divergence, no conflict copy; rerun on identical code GREEN → timing race). Two real defects fixed: (1) apply_entry's shared put_file clobbered the row's DISK stat with `server_ts` on same-manifest no-op replays — the very next remote-differing upsert then read as drifted (the round-13 guard compares that field); rows now carry their own stat in every content-this-device-has arm. (2) even a genuine external stat-only touch (the on-runner toucher is unidentified — CfAPI conversion / metadata-normalizing I/O are the candidates) used to consume the refused remote with no re-delivery: process_file now short-circuits when re-hashed content is identical to the row's manifest — refresh the stat, re-pin replay to the fork point (the conflict_copy re-delivery, minus the copy), converge in the same pass. 2 regression tests (`remote_same_manifest_replay_preserves_row_stat`, `stat_only_drift_converges_via_fork_repin`); 470 workspace tests green. |
| e2e deadline headroom (post-round CI) | ✅ | the `test` job flaked on 49d65a9 (`b fetched` at the 15 s per-block deadline, zero p2p code in the commit; 6/6 local passes ~4.2 s; rerun GREEN on identical code): e2e PROGRESS deadlines now 60 s fetch / 30 s convergence — pure failure-path headroom, happy path unchanged; NEGATIVE deadlines (no-holder 2 s, stranger-gate 2 s, 4 s observation window) deliberately untouched. Policy + flake evidence in the e2e.rs module doc. |
...
| Gates | ✅ | `node --check` both JS files; every `getElementById` cross-checked against markup (zero dangling IDs); `cargo fmt` clean; windows-target check on cairn-tray; cairn-cli (7) + cairn-review (17) + cairn-proxy (11) test suites green (assets are `include_str!` — compile = ship). **Live browser verification** (headless Chromium against a mock daemon serving the REAL assets): dashboard — onboarding rail step states over real fields, all panels populated, zero console/page errors; review portal — boot lands on newest version, JKL shuttle (L plays rate 1→8, J rewind ticker rewound 96→48 fr, K stops), shift+arrow ×10 frame steps, zoom 1x→2x→4x HUD, comment submit appends + re-renders, open/done filters, v2 switch dims/undims correctly, honest waveform VISIBLE (real 6s test media with audio decoded). VLM design review of both screenshots: zero layout defects, "highly polished / professional". One post-push CSS guard added: `[hidden] { display:none !important }` (author display rules were overriding the UA hidden attr on icon-btn svg / notfound / composer / wave). |
...
| Kani proof shards (cairn-tl heavy pair) | ✅ hardened (round 14: stubs + sharding + drop-glue scoping) | the named follow-up LANDED, three moves, one empirically-tuned bound: (1) STUBS — `bounded_op`'s fixed String/serde_json construction replaced with allocation-free `stub_string`/`stub_value`/`stub_marker` (sound: the classifier never branches on string/JSON CONTENT in the bounded model, and both sides construct stubs identically so every equality outcome — key==key, value==value, marker==marker — matches the original fixed-content model); (2) PER-KIND SHARDING — the two >90-min harnesses split into 11+11 `proof_{classifier,symmetry}_shard_kind_00..10` harnesses (ours-kind a concrete constant, theirs-kind symbolic; the union of each family's 11 shards is EXACTLY the original 121-pair space, so no coverage is lost — each CBMC job explores 11 pairs instead of 121; names zero-padded because `--harness` substring-matches, so `kind_1` would silently double-run `kind_10`); (3) DROP-GLUE SCOPING — live evidence from sharded run 33842890616 showed the remaining blowup was CBMC exploring `drop_in_place` for the ops' String/BTreeMap/Value fields (allocator machinery — even `interacts`-only harnesses ground >18 min on it), so the harnesses now `mem::forget` the ops and verdict after asserting, the standard Kani move when destructors are not part of the property (real drop behavior stays pinned by the ordinary test suites). After (1)-(3) each shard verifies in ~20–27 s (was >90 min for the un-split harness). Bound tuning from run 33844155416: `kind_10` (TrackReorder) failed ONLY `memcmp.unwind.0` — the model's `Vec<usize>` order compare is 16 bytes vs unwind 12, the same class the cairn-core commit shard solves with unwind 104 — so the cairn-tl shards run `--default-unwind 32`. kani.yml: 27 parallel runners, `continue-on-error` soft-gate REMOVED — both families HARD. History that drove this: every GitHub-hosted attempt since round 12 was killed by RUNNER SHUTDOWN at 50–95 min mid-exploration — never a verification failure, never completed (record 94 min); that evidence justified the temporary soft-gate and names this fix as the hardening path. The classifier logic itself remains additionally pinned by the 89 cairn-tl tests + the 18-file real-timeline corpus gate |
