...
| Kani proof shards (cairn-tl heavy pair) | 🟨 infra-limited, soft-gated | `proof_classifier_total_and_panic_free` + `proof_interaction_symmetry` explore serde_json/String allocation internals under unwind 12 (>90 min solver time). Every GitHub-hosted attempt since round 12 was killed by RUNNER SHUTDOWN at 50–95 min — never a verification failure, never completed (tonight's dispatch survived 94 min, the record). Evidence-driven call (kani.yml comment): the 5 cheap/BLAKE3 shards stay HARD-gated and green; the two heavy shards stay in the nightly matrix (a calm window can land them) but `continue-on-error` so preemption cannot red the gate. Hardening path (known follow-up): Kani stubs for String/serde_json construction in `bounded_op`. The classifier logic itself is pinned by 89 cairn-tl tests + the 18-file real-timeline corpus gate |
