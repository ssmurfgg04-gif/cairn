...
Part 2: the NLE matrix moved from "plan + collector" to **executed in CI**
(`.github/workflows/nle-matrix.yml`, weekly + push-triggered):

| Item | Status | Evidence |
|---|---|---|
| Real-NLE timeline corpus (part 1) | ✅ | 18 pinned timelines (python-otio production samples + authentic FCP X FCPXML from PRONOM/BBC/cutlass) × tl-capture × both merge contracts × merged-output recapture × interop oracle; outcome pins fail on drift in BOTH directions; local run 17/18 green (big_int.otio honestly refused — python's non-Inf JSON tokens) → `docs/nle-matrix-results/real-timeline-corpus.json`; now also a CI job. It caught 3 real engine bugs live: FCPXML descriptor subtrees dropped (4/5 real files refused — now preserved verbatim in `extra[fcpxml:<tag>]`), OTIO `Sequence.1` schema children dropped, matched-track attr edits invisible to the diff (`Op::TrackAttr` added, C2/C3/C6/C9 arms, Kani 11 kinds) |
| Two-device CfAPI stack matrix (W0–W6) | ✅ shipped | `scripts/win_nle_matrix.ps1` + `win_nle_stack.ps1`: real server + two daemons + two sync roots on windows-latest; BMW27.blend authored through A → cold-2-MiB I1 + SHA256 identity on B → **headless Blender open/scrub/save/reopen through B's root** → cross-device byte convergence → deterministic H9 conflict (offline-edit vs live-edit → ONE copy, both recoverable) → Windows-binary tl-merge exit contract. All rows + metrics land in one JSON (artifact + job summary). Linux machinery dry run: W0/W1/W2/W5/W6 green end-to-end (real two-device sync + real conflict), W3 is the CI-only leg (bpy) |
| **Engine bugs W5 caught LIVE (both fixed + regression-pinned)** | ✅ | (1) `materialize_missing` clobbered UNDISCOVERED local edits: a remote update for a locally-clean-but-drifted row overwrote the local bytes with NO conflict copy (edit-discovery lag: plain-file workspaces discover edits only at the periodic scan) → apply-time stat guard with the scan's EXACT predicate re-dirties the row instead (3 unit tests); (2) append `base_seq` used the READ CURSOR, conflating "what I've seen" with "what my bytes descend from": the forked append was accepted LINEARLY, silently superseding the other device's version with no copy → content-lineage FORK markers (`fork:<project>:<path>`), appends claim `min(cursor, fork-1)`, and conflict resolution re-pins the journal replay to the fork point so the original path re-receives the winner. Pinned end-to-end in `cairn-sim::w5_tests::conflict_copy_survives_edit_discovery_lag` (real server, real journal conflict rule: ONE copy, original converges to the winner on BOTH devices, copy carries the loser) |
| "Install DaVinci on the runner" (the explicit ask) | 🟨 probed | `scripts/win_resolve_probe.ps1`: winget search → silent install → launch, recorded honestly either way; non-blocking (outcome is data about the RUNNER). If Resolve cannot start GPU-less, H4/H5 stay studio legs and Blender+corpus carry CI realism |
| PS 5.1 / script bugs caught while writing it | ✅ fixed | `Wait-ProjectSynced` grepped field names that don't exist in `status --json` (always times out — now parses JSON); `$HOME` is a read-only automatic variable (every `-Home` param binding threw — renamed `-HomeDir`); `cairn status` takes NO `--ctl` flag (clap exit 2 — removed); dot-sourcing `win_nle_stack.ps1` reset its `param($Cairn)` (now passed explicitly); uncaptured `Wait-TcpPort` returns polluted function output ([bool, Process] instead of the handle); `ProcessStartInfo.ArgumentList` doesn't exist on .NET Framework (runtime-detected + quoting fallback); native-stderr-under-EAP-Stop traps avoided via the Process API; WaitForExit pipe deadlocks avoided with ReadToEndAsync; conflict-copy filename carries U+2014 — JSON written BOM-less UTF-8, match patterns stay ASCII |
