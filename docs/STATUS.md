...
| CI gate through a LIVE mount | ✅ `blender-headless` job | New job in fuse-mount-live.yml (ubuntu-latest — these VMs have /dev/fuse; runs on every push touching fs-linux/store/chunker/harness paths + ubuntu dispatches): build cairn-fuse → mount a fresh store → **seed BMW27.blend THROUGH the mount with a sha256 byte-identity assert** (write-back ingest + read-back) → run the harness against `/tmp/cairn-blender/mnt/project/scene.blend` for 2 rounds × 120 frames → unmount → store-persistence assert → `always()` cleanup. Self-hosted dispatches skip it (no Blender requirement on the dedicated fuse box) |
