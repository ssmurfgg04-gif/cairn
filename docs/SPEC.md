...
| ADR-0019 | Round-15 production-usability five: CfAPI async population + watchdog, multi-root single-daemon (root namespaces + root-qualified authorship), OTIO pre-ingestion leniency, mDNS LAN discovery (code-fingerprint beacons), Windows shell extension (overlay icons + context menu) |
| ADR-0020 | Round-16 review loop: client review portal (guest links, frame-accurate comments, version stacks, presence), proxy workflow (digest-keyed project files + pluggable transcoders), RBAC (roles × permissions + members.json), NLE marker bridge (OTIO + FCP7 XML), AAF/OMF handoff ledger (file digest + timeline digest binding) |
| ADR-0022 | Round-18: dogfood-found review-loop bugs fixed (marker TC rate from the version, latest_only visibility, proxy fallback + even-dimension generation + ffprobe publish), daemon-side RBAC at every mutating ctl handler with a synced audit ledger, the Tauri-2 window crate (workspace-excluded, tauri-check CI), the 12-point UI audit (files/team/search/quota/help/i18n/hero-onboarding), WAN STUN wiring + runbook, tray balloons, association-based shell-ext open |
| ADR-0021 | Round-17 design system: one token set, three surfaces (light dashboard / dark review portal / native tray), the reference aurora-glass-blue-DM-Sans language at taste-skill discipline; review player mechanics (JKL shuttle, 400% zoom+pan, honest ≤40 MiB waveform, filters, live composer TC); build-free and offline-degrading by construction |
