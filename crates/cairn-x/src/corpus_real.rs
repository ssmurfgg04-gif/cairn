...
pub fn run(dir: &str, out_json: &str, mutation_file: Option<String>) -> anyhow::Result<()> {
    ...
    for p in &paths {
        ...
        let uniq: HashSet<String> = sh.chunk_hashes.iter().map(|h| h.hex().clone()).collect();
...
fn mutation_reuse(path: &std::path::Path) -> anyhow::Result<(f64, usize)> {
    ...
    let h1: HashSet<String> = sh1.chunk_hashes.iter().map(|h| h.hex().clone()).collect();
    let shared = sh2
        .chunk_hashes
        .iter()
        .filter(|h| h1.contains(&h.hex().clone()))
        .count();
