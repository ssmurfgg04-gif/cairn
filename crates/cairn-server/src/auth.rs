...
impl Authenticator {
    ...
    pub async fn enroll_code(
        ...
        self.codes.write().await.insert(
            code.clone(),
            (tenant_id.into(), email.into(), scopes.into(), exp),
        );
        code
    }
    ...
    pub async fn authenticate(
        ...
        let claims = trusted.payload_claims().ok_or_else(|| {
            CairnError::new(ErrorKind::Unauthenticated, "token without payload claims")
        })?;
        ...
        if stored_hash != token_hash {
            return Err(CairnError::new(
                ErrorKind::Unauthenticated,
                "token does not match device enrollment",
            ));
        }
...
mod tests {
    ...
    async fn setup() -> (tempfile::TempDir, SqlitePool, Authenticator) {
        ...
        let pool = db::open(&std::path::Path::new(dir.path()).join("meta.db"))
            .await
            .unwrap();
        db::migrate(&pool).await.unwrap();
        let auth =
            Authenticator::load_or_create(dir.path(), Arc::new(cairn_core::clock::WallClock))
                .unwrap();
        (dir, pool, auth)
    }
    ...
    async fn enroll_authenticate_roundtrip() {
        ...
        let code = auth
            .enroll_code("t1", "editor@studio", "sync", 60_000)
            .await;
        let (token, identity) = auth
            .enroll(&pool, &code, "pubkey-hex", "bench-a")
            .await
            .unwrap();
        assert_eq!(identity.tenant_id, "t1");
        let verified = auth
            .authenticate(&pool, &format!("Bearer {token}"))
            .await
            .unwrap();
