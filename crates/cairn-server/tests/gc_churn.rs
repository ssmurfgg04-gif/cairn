...
async fn gc_shadow_zero_violations_over_10k_churn() {
    ...
    for round in 0..50u64 {
        for f in 0..files {
            ...
            let mh = Hash::of(&body);
            state.store.put(&cairn_server::storage::LocalFsStore::object_key("t1", &mh.hex()), &body).await.unwrap();
            ...
            state.store.put(&cairn_server::storage::LocalFsStore::chunk_key("t1", &ch.hex()), &chunk).await.unwrap();
            sqlx::query("INSERT OR IGNORE INTO chunks(tenant_id, hash, size, tier, state, last_touched) VALUES('t1',?1,?2,'hot','present',0)")
                ...
                .execute(&state.db).await.unwrap();
            let entries = vec![ManifestEntry { offset: 0, len: chunk.len() as u32, chunk_hash: ch }];
            let m = Manifest::build(entries, Compression::None, None);
            let (_mh2, mb) = m.serialize();
            let _ = mb;

            base += 1;
            cairn_server::journal::append(&state.db, &state.clock, "t1", "p1", "d1", &format!("u-{round}-{f}"), upsert(&format!("f{f}.bin"), &mh.hex(), base - 1), 0).await.unwrap();
            if round > 0 {
                let del = cairn_proto::pb::JournalOp {
                    op: Some(OpKind::FileDelete(cairn_proto::pb::FileDeleteOp { path: format!("f{f}.bin"), base_seq: base - 1 })),
                };
                base += 1;
                cairn_server::journal::append(&state.db, &state.clock, "t1", "p1", "d1", &format!("d-{round}-{f}"), del, 0).await.unwrap();
            }
        }
    }

    // fold so refs exist (GC roots), then shadow GC
    let (_commit, _) = cairn_server::fold::fold(&state, "t1", "p1", "canary", "pre-gc").await.unwrap();
    let (flagged, violations, scanned) = cairn_server::jobs::gc::gc_pass(&state, "t1", true).await.unwrap();
    assert_eq!(violations, 0, "GC shadow MUST report zero violations (I2/(d))");
    assert_eq!(scanned, 5000); // 50 rounds x 100 files, one chunk each
    assert!(flagged > 0, "churn created unreachable chunks that GC flags");
    assert!(violations == 0);
